...
async function main() {
  ...
  manager.addNamedEntityText('duration', 'daily', ['en'], ['daily', 'a day']);
  manager.addNamedEntityText('duration', 'monthly', ['en'], ['monthly', 'a month']);
  ...
  manager.addNamedEntityText('duration', 'quotidien', ['fr'], ['quotidien', 'par jour']);
  manager.addNamedEntityText('duration', 'mensuel', ['fr'], ['mensuel', 'par mois']);
  ...
  //
  const result1 = await manager.process('fr', 'Je veux augmenter mon plafond mensuel de paiement de ma carte bancaire', conversationContext);
  console.log(result1.answer, conversationContext);

  const result3 = await manager.process('fr', '500', conversationContext);
  console.log(result3.answer, conversationContext);

  const result4 = await manager.process('fr', 'oui', conversationContext);
  console.log(result4.answer, conversationContext);

  conversationContext = {};

  const result5 = await manager.process('en', 'I want to increase the daily payment limit of my card', conversationContext);
  console.log(result5.answer, conversationContext);

  const result6 = await manager.process('en', '500', conversationContext);
  console.log(result6.answer, conversationContext);
